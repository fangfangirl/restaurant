...
{
    $name = $_SESSION['id_r'];
    $sql = "SELECT * FROM user_restaurant WHERE username_res='$name' and password='$oldpassword'";
    ...
    {
        alert2('輸入舊密碼錯誤，請重新確認','../view/update_ResPassword.php');
    }
    else if( $newpassword == $db_password )
    {
        alert2('新舊密碼不可以相等','../view/update_ResPassword.php');
    }
    else{
        $row = mysqli_fetch_array($res,MYSQLI_NUM); //把$res转换成索引数组
        $update="UPDATE user_restaurant SET password='$newpassword' WHERE username_res='$name'";
